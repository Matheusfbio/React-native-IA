...
export function Assistant() {
  ...
  async function generateGeminiResponse() {
    try {
      ...
      const currentMessages = [...geminiResponse, localInput];
      setGeminiResponse(currentMessages);
      ...
      if (fileCopy) {
        ...
        response = await fetch(`${DOMAIN}/chat/gemini`, {
          method: "POST",
          body: formData,
        });
      } else {
      ...
      setGeminiResponse([
        ...
        {
          type: "assistant",
          value: text || "Resposta vazia do servidor",
        },
      ]);

      setLoading(false);
      console.log('=== GEMINI REQUEST FINALIZADO ===');
    } catch (err) {
      console.log("=== ERRO NO GEMINI ===:", err);
      setLoading(false);
    }
  }
  ...
  const isStarted = geminiResponse.length;
        ...
        {item.type === "user" && (
          <View style={styles.promptTextContainer}>
            <View style={styles.promptTextWrapper}>
              <Text style={styles.promptText}>{item.value}</Text>
            </View>
          </View>
        )}
        {item.type === "assistant" && (
          <View style={styles.textStyleContainer}>
            <Markdown style={styles.textStyle}>{item.value}</Markdown>
            <TouchableHighlight
    ...
    >
          ...
          <View style={styles.midChatInputWrapper}>
            <View style={styles.midChatInputContainer}>
              ...
              <TextInput
                ...
                style={styles.midInput}
                placeholder="Instruções para o assistente (opcional)"
                placeholderTextColor={theme.placeholderTextColor}
                autoCorrect={true}
                value={instructions}
              />
              ...
              >
                <View style={styles.midButtonStyle}>
                  ...
                  />
                  <Text style={styles.midButtonText}>Chat</Text>
                </View>
              </TouchableHighlight>
              {file && (
              ...
              <Text style={styles.chatDescription}>
                Chat com Gemini (com instruções opcionais e interpretação de
                arquivos)
              </Text>
            </View>
          </View>
      ...
      {Boolean(isStarted) && file && (
        <View style={styles.fileNameContainer}>
          <Text style={styles.fileName}>{file.name || "File from device"}</Text>
          <TouchableHighlight
            onPress={() => setFile(null)}
            style={styles.closeIconContainer}
            underlayColor={"transparent"}
      ...
      {Boolean(isStarted) && (
        <View style={styles.chatInputContainer}>
          <TextInput
            onChangeText={onChangeInputText}
            style={styles.input}
            placeholder="O que mais você quer saber?"
            placeholderTextColor={theme.placeholderTextColor}
            autoCorrect={true}
            value={input}
          />
